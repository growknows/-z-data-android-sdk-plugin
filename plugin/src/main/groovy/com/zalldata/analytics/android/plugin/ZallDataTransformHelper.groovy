...
class ZallDataTransformHelper {
    ...
    private void createZallDataHookConfig() {
        ...
        for (it in metaProperties) {
            ...
            if (extension.sdk."${it.name}") {
                zallAnalyticsHookConfig."${it.name}"(it.name)
            }
        }
    }
    ...
    ClassNameAnalytics analytics(String className) {
        ...
        if (classNameAnalytics.isSDKFile()) {
            def cellHashMap = zallAnalyticsHookConfig.methodCells
            cellHashMap.each {
                key, value ->
            ...
            if (classNameAnalytics.methodCells.size() > 0 || classNameAnalytics.isZallDataAPI
