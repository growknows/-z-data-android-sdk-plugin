...
public class ZallDataPushMethodVisitor extends AdviceAdapter {
    ...
    public void visitMethodInsn(int opcodeAndSource, String owner, String name, String descriptor, boolean isInterface) {
        ...
        if (opcodeAndSource == Opcodes.INVOKESTATIC && PENDING_INTENT_OWNER.equals(owner) && checkPendingIntentName(name)) {
            ...
            mv.visitMethodInsn(Opcodes.INVOKESTATIC, ZallPushInjected.PUSH_TRACK_OWNER, getIntentHookMethodName(argTypes.length, name),
                    refactorHookBeforeMethodDescriptor(descriptor), false);
            positionList.forEach(this::loadLocal);
            super.visitMethodInsn(opcodeAndSource, owner, name, descriptor, isInterface);
            mv.visitInsn(Opcodes.DUP);
            positionList.forEach(this::loadLocal);
            mv.visitMethodInsn(Opcodes.INVOKESTATIC, ZallPushInjected.PUSH_TRACK_OWNER, getPendingIntentHookMethodName(argTypes.length, name),
