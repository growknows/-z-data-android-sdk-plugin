...
class ZallDataTransform extends Transform {
    ...
    // “com.zalldata.analytics.android.sdk.ZallDataAPI” 类所在路径
    ...
    private boolean checkJarValidate(JarInput jarInput) {
        try {
            if (isFoundSDKJar || zallSdkJarPath == null) {
                return false
            }
            def jarLocation = jarInput.file.toURI().getPath()
            if (zallSdkJarPath.length() == jarLocation.length() && zallSdkJarPath == jarLocation) {
                isFoundSDKJar = true
                return true
            } else {
                return false
