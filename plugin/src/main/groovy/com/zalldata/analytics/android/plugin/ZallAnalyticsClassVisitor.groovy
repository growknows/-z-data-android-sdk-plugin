    ...
    void visitEnd() {
        ...
        if (ZallAnalyticsUtil.isInstanceOfFragment(mSuperName)) {
            ...
            while (iterator.hasNext()) {
                ...
                mv.visitEnd()
                mv.visitAnnotation("Lcom/zalldata/analytics/android/sdk/ZallDataInstrumented;", false)
            }
        }

        for (cell in methodCells) {
            transformHelper.zallAnalyticsHookConfig."${cell.agentName}"(classVisitor, cell)
        }
        if (Logger.debug) {
        ...
        ZallAnalyticsDefaultMethodVisitor zallAnalyticsDefaultMethodVisitor = new ZallAnalyticsDefaultMethodVisitor(methodVisitor, access, name, desc) {
            ...
            void visitEnd() {
                ...
                if (isHasTracked) {
                    ...
                    visitAnnotation("Lcom/zalldata/analytics/android/sdk/ZallDataInstrumented;", false)
            ...
            void handleCode() {
                if (isHasInstrumented || classNameAnalytics.isZallDataAPI) {
                    return
                }
                ...
                if (isZallDataIgnoreTrackOnClick) {
                    return
                }
                ...
                if (isZallDataTrackViewOnClickAnnotation && desc == '(Landroid/view/View;)V') {
                    trackViewOnClick(methodVisitor, 1)
                    isHasTracked = true
                    return
                }
                ...
                switch (transformHelper.rnState) {
                    ...
                    case ZallAnalyticsTransformHelper.RN_STATE.HAS_VERSION:
                        if (transformHelper.rnVersion > '2.0.0' && mSuperName == "com/facebook/react/uimanager/ViewGroupManager"
                            ...
                            methodVisitor.visitVarInsn(ALOAD, 2)
                            methodVisitor.visitVarInsn(ILOAD, 3)
                            methodVisitor.visitMethodInsn(INVOKESTATIC, "com/zalldata/analytics/RNAgent", "addView", "(Landroid/view/View;I)V", false)
                            result = true
                        }
                        ...
                        if (nameDesc == 'handleTouchEvent(Landroid/view/MotionEvent;Lcom/facebook/react/uimanager/events/EventDispatcher;)V' && mClassName == 'com/facebook/react/uimanager/JSTouchDispatcher') {
                            ...
                            methodVisitor.visitVarInsn(ALOAD, 1)
                            methodVisitor.visitVarInsn(ALOAD, 2)
                            methodVisitor.visitMethodInsn(INVOKESTATIC, "com/zalldata/analytics/RNAgent", "handleTouchEvent", "(Lcom/facebook/react/uimanager/JSTouchDispatcher;Landroid/view/MotionEvent;Lcom/facebook/react/uimanager/events/EventDispatcher;)V", false)
                            result = true
                        }
                        break
            ...
            AnnotationVisitor visitAnnotation(String s, boolean b) {
                if (s == 'Lcom/zalldata/analytics/android/sdk/ZallDataTrackViewOnClick;') {
                    isZallDataTrackViewOnClickAnnotation = true
                    Logger.info("发现 ${name}${desc} 有注解 @ZallDataTrackViewOnClick")
                } else if (s == 'Lcom/zalldata/analytics/android/sdk/ZallDataIgnoreTrackOnClick;') {
                    isZallDataIgnoreTrackOnClick = true
                    Logger.info("发现 ${name}${desc} 有注解 @ZallDataIgnoreTrackOnClick")
